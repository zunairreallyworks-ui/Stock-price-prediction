...
function persistSelections() {
  ...
  localStorage.setItem("rf_selected_symbol", symbol);
  localStorage.setItem("rf_selected_split", split);
}

function restoreSelections() {
  const savedSymbol = localStorage.getItem("rf_selected_symbol") || "AAPL";
  const savedSplit = localStorage.getItem("rf_selected_split") || "80";
...
function sendTrainingSplit() {
  ...
  Chart.getChart("rfChart")?.destroy();
...
function loadRFChart(symbol) {
  const timestamp = Date.now();

  fetch(`/api/random-forest-predictions/${symbol}?t=${timestamp}`, {
    cache: "no-store",
  })
    .then((res) => res.json())
    .then(updateChart)
  ...
  fetch(`/api/random-forest-scores/${symbol}?t=${timestamp}`, {
    cache: "no-store",
  })
    .then((res) => res.json())
    .then((data) => {
