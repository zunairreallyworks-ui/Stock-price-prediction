...
function loadChart(symbol) {
  ...
  fetch(`/api/svm-predictions/${symbol}?t=${timestamp}`)
    .then((res) => res.json())
    .then(updateChart)
    .catch((err) => {
  ...
  fetch(`/api/svm-scores/${symbol}?t=${timestamp}`)
    .then((res) => res.json())
    .then((data) => {
      document.getElementById("mae").innerText = data.mae;
      document.getElementById("rmse").innerText = data.rmse;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("trainSlider").addEventListener("change", trainModel);
  document
    .getElementById("symbolSelector")
    .addEventListener("change", trainModel);
});
